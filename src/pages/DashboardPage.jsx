  ...
  return (
    <div className="min-h-screen pt-20 pb-16">
      <div className="max-w-5xl mx-auto px-4 py-8">
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <div className="text-center py-16">
                ...
                <Link to="/products" className="text-primary-400 hover:text-primary-300 transition-colors">Start shopping →</Link>
              </div>
            ) : (
              orders.map((order, i) => (
                <motion.div key={order._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium capitalize ${statusColors[order.status] || 'bg-gray-700 text-gray-300'}`}>{order.status}</span>
                  ...
                  <div className="flex gap-3 overflow-x-auto scrollbar-hide">
                    {order.orderItems.slice(0, 4).map((item, j) => (
                      <div key={j} className="w-12 h-12 rounded-lg overflow-hidden bg-dark-700 shrink-0">
                        <img src={item.image} alt={item.name} className="w-full h-full object-cover" onError={e => e.target.src='https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=50'} />
                      </div>
                    ))}
                    {order.orderItems.length > 4 && <div className="w-12 h-12 rounded-lg bg-dark-700 shrink-0 flex items-center justify-center text-xs text-gray-400">+{order.orderItems.length - 4}</div>}
                  </div>
                </motion.div>
              ))
            )}
        ...
        {activeTab === 'profile' && (
          <motion.form initial={{ opacity: 0 }} animate={{ opacity: 1 }} onSubmit={handleProfileUpdate}
            ...
            <div>
              ...
              <input type="text" value={profileForm.name} onChange={e => setProfileForm(p => ({ ...p, name: e.target.value }))} required
            ...
            <div>
              ...
              <input type="email" value={user?.email} disabled className="w-full bg-dark-700/50 border border-white/5 rounded-xl px-4 py-3 text-sm text-gray-500" />
            </div>
            <div>
            ...
            <button type="submit" disabled={saving} className="bg-primary-500 hover:bg-primary-400 disabled:opacity-60 text-dark-900 font-bold px-6 py-3 rounded-xl transition-colors">
