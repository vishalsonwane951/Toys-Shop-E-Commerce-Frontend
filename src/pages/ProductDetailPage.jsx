...
const StarRating = ({ rating, interactive = false, onRate }) => (
  <div className="flex gap-1">
    {[1,2,3,4,5].map(s => (
      <button key={s} onClick={() => interactive && onRate?.(s)} disabled={!interactive}
        className={`${interactive ? 'cursor-pointer hover:scale-110 transition-transform' : ''}`}>
        <svg className={`w-5 h-5 ${s <= Math.round(rating) ? 'text-primary-400' : 'text-gray-600'}`} fill="currentColor" viewBox="0 0 20 20">
...
export default function ProductDetailPage() {
  ...
  const imgs = product.images?.length > 0 ? product.images : ['https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=600&q=80'];
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
            <div>
              <span className="text-primary-400 text-sm font-medium uppercase tracking-widest">{product.category?.name}</span>
            ...
            <div className="flex items-baseline gap-3">
              ...
              {product.originalPrice && <>
                ...
                <span className="bg-red-500/20 text-red-400 text-sm font-medium px-3 py-1 rounded-full">Save {discount}%</span>
              </>}
            </div>
            ...
            {product.stock > 0 && (
              <div className="flex items-center gap-4">
                ...
                <button onClick={handleAddToCart} className="flex-1 bg-primary-500 hover:bg-primary-400 text-dark-900 font-bold py-3 px-6 rounded-xl transition-colors flex items-center justify-center gap-2">
                ...
                product.reviews.map((r, i) => (
                  <div key={i} className="bg-dark-800 border border-white/10 rounded-2xl p-5">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 rounded-full bg-primary-500/20 flex items-center justify-center text-sm font-bold text-primary-400">{r.name[0]}</div>
